# Central Bank of Kuwait Exchange Rate API client

Official **Central Bank of Kuwait** (Kuwait) daily exchange rates in Node.js / TypeScript — 10 currencies against the KWD, with history back to 2008. Zero dependencies, works in Node 18+, Bun, Deno, and edge runtimes (uses global `fetch`).
...
Powered by [AllRatesToday](https://allratestoday.com/central-bank-rates-api/cbk/). Get a free API key at [allratestoday.com/register](https://allratestoday.com/register) — 300 requests/month, no credit card.
...
console.log(pair.rate, pair.rate_date); // e.g. USD -> KWD on the bank's own date
...
If Central Bank of Kuwait does not print a pair directly, the API resolves it from the bank's table (inverse, or a cross rate via KWD) and flags it `derived: true` with the `method` — so official and computed values are never confused.
